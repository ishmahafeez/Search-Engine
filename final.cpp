...
class ll //completely for history
{
    public:
    NodeS* first;
    ll()
 	...
 	public:
        ...
        void search(ll* history)
        {
            ...
            while(returned != -1)
            {
                ...
                int filenum;
                int ind;
                filenum = returned/10000; //through hashing
                //ind ko dekhna
                ind = returned%10000; 
                if(ind == 0)
                {
                ...
                filename+= ".txt"; //opens the hashed index file
                ifstream inner;
                string line;
                inner.open(filename);
...
{   
	...
	//cout<<"ABL :"<<av1->root->data<<"index "<<av1->root->index<<endl; //Gleneagles Medical Centre   
	while(option != 6){
        cout<<"\n\t\t\t   _____";								// Prints menu.
	    cout<<"\n\n\t\t\t     Main Menu";
	    cout<<"\n\t\t\t   _____";		
